(globalThis.TURBOPACK||(globalThis.TURBOPACK=[])).push(["object"==typeof document?document.currentScript:void 0,65880,e=>{"use strict";e.s(["buildBlackBoxRevealPrompt",0,function(e,t,n){return{systemPrompt:`# 角色
...
"""
${t}
"""`}},"buildCardConclusionPrompt",0,function(e,t,n){let s=`# 角色
...
话题：${t}
AI 面试评分：${n.score??"无"} 分
AI 总评：${n.comment??"无"}`,o=n.followup_qa_pairs?.length?"\n\n【追问环节】\n"+n.followup_qa_pairs.map((e,t)=>`追问${t+1}：${e.question}
回答：${e.answer}`).join("\n\n"):n.followup_answers?.length?"\n\n追问回答：\n"+n.followup_answers.map((e,t)=>`追问${t+1}：${e}`).join("\n"):"";return{systemPrompt:s,userPrompt:`【用户原始回答】
${n.original_answer??"无"}${o}`}},"buildDebriefPrompt",0,function(e,t,n,s,o,a){return{systemPrompt:`# 角色
你是懂面鹅，在用户亲手拆完一个 AI 面试黑箱后，帮 ta 把这段经历沉淀成一个有意义的领悟。

# 上下文
- 话题：${e}
- 用户最初选了：选项 ${["A","B","C","D"][n]}
- 最后选了：${void 0!==s?"选项 "+["A","B","C","D"][s]:"未重投"}
- 正确答案：选项 ${["A","B","C","D"][o]}
- 立场是否反转：${void 0!==s&&n!==s?"是，改变了自己的判断":"否，坚持了最初的判断"}
- BBSI 评分：${a?JSON.stringify(a):"无"}
...
# 输出
仅 JSON：{ "insight": "...", "journey": "...", "takeaway": "..." }`,userPrompt:`用户实测回答：
"""
${t}
"""`}},"buildDynamicBlackBoxPrompt",0,function(e){return{systemPrompt:`# 角色
...
}`,userPrompt:`用户的担忧：${e}`}},"buildEmpathyPrompt",0,function(e){return{systemPrompt:`# 角色
...
# 输出
纯文本，无 JSON，无引号包裹。`,userPrompt:`用户说：${e}`}},"buildFollowupsPrompt",0,function(e){return{systemPrompt:`# 角色
...
# 输出
仅 JSON，无包裹：
{ "followups": [ { "question": "...", "targets": "action" } ] }`,userPrompt:`候选人回答：
"""
${e}
"""`}},"buildHandbookPrompt",0,function(e,t){return{systemPrompt:`# 角色
你是腾讯 BBSI 面试教练，给完成多轮 AI 面试实测的候选人写一份有温度、有干货的通关手册。你不是在写绩效评估，而是在帮一个认真对待自己未来的人看清优势、补上短板。

# 上下文
- 用户完成话题数：${e.length}
- 各维度累计得分：${JSON.stringify(t)}
...
仅 JSON，无包裹：
{
  ...
  "value_assessment": "...",
  "encouragement": "..."
}`,userPrompt:`用户完成的题目：${e.map(e=>e.keyword).join("、")||"（无）"}`}},"buildInterviewChatMessages",0,function(e,t){return[{role:"system",content:`# 角色
...
# 结束信号
用户回答中出现完整 STAR + 量化结果 / 主动提及反思时，结束面试并给 2-3 句简短反馈`},...e]},"buildInterviewQuestionPrompt",0,function(e,t){return{systemPrompt:`# 角色
...
- 任务要和「${e}」这个话题自然关联，但出发点必须是"让 ta 体验 AI 评分"而非"考 ta"
- 设计思路参考：
  \xb7 如果话题关于口音/表达 → 设计一段让 ta 自然讲述的话（比如"用你最舒服的方式介绍一个你做过的项目"）
  \xb7 如果话题关于公平/偏见 → 设计一个让 ta 展示真实经历的题目（让 AI 证明它只看内容不看背景）
  \xb7 如果话题关于紧张/发挥 → 设计一个轻量小任务（让 ta 看到 AI 对"不完美回答"的真实反应）
  \xb7 如果话题关于学历/背景 → 设计一个展示能力的题目（让评分说话，而非简历说话）
...
# 输出
仅 JSON，无包裹：
{ "question": "<实战任务>" }`,userPrompt:`话题：${t}
关键词：${e}`}},"buildInterviewScorePrompt",0,function(e,t,n){let s=n.length>0?n.map((e,t)=>`追问${t+1}：${e.question}
回答${t+1}：${e.answer}`).join("\n\n"):"（无追问）";return{systemPrompt:`# 角色
...
请严格按照六维评分体系对以上完整 Q&A 进行评分。记住：你的评分不仅是打分，更是让候选人理解 AI 面试评分逻辑的机会。`}},"buildMatchReasonPrompt",0,function(e,t){return{systemPrompt:`# 角色
...
# 输出
纯文本，无 JSON 包裹，无引号。`,userPrompt:`用户担忧：${e}
匹配话题：${t}`}},"buildMatchTopicPrompt",0,function(e,t){return{systemPrompt:`# 角色
...
# 输出
仅 JSON，无任何包裹与解释：
{ "matched_topics": ["..."], "guidance": "..." }`,userPrompt:`用户担忧原文：${e}`}},"buildRewritePrompt",0,function(e,t,n){return{systemPrompt:`# 角色
...
- 针对「${t}」维度做强化：逻辑思维=加结构词 / 问题解决=加方案与约束 / 沟通协作=加跨角色互动 / 价值观匹配=加进取与成长信号
...
"""
${e}
"""`}},"buildScorePrompt",0,function(e,t){return{systemPrompt:`# 角色
...
- STAR 完整度强制约束（关键）：
  \xb7 缺 A（个人具体行动）→ 所有维度上限 3 分
  \xb7 缺 R（量化/可验证结果）→ 所有维度上限 6 分
...
- truth 与 mechanism 紧扣关键词「${t}」这一面试焦虑本身做解读，不要复述用户回答
...
仅输出 JSON，禁止包裹 \`\`\`json、禁止任何解释文字、禁止开场白。严格遵循下述 schema：
{
  ...
  "truth": "<针对「${t}」这一 AI 面试担忧的真相解读，80-150 字，中立客观>",
